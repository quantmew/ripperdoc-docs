...
说明：
- 文件超过 256KB 或行数超过 2000 时需要使用 `offset`/`limit`。
- 可通过 `RIPPERDOC_MAX_READ_FILE_SIZE_BYTES` 和 `RIPPERDOC_MAX_READ_LINES` 调整限制。
...
创建一个隔离的 git worktree 并将会话切换到其中，以便安全地进行实验。
...
- 为安全实验创建隔离的 worktree
- 如未提供名称则自动命名
- 需要 git 仓库或 WorktreeCreate/WorktreeRemove hooks
...
持久化记忆文件工具，用于在会话之间存储和检索知识。

```
查看记忆：patterns
创建记忆：patterns.md 包含内容
替换记忆：patterns 中的 old_string 为 new_string
插入记忆：patterns 在第 10 行
删除记忆：old-pattern
重命名记忆：old-name 为 new-name
```
...
- 支持创建/替换/插入/删除/重命名操作
- 基于行的插入和替换
- 可选 view_range 用于读取特定行范围
- 带有权限 bypass 的自动记忆系统，用于上下文保留
