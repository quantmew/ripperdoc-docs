...
### RIPPERDOC_ 环境变量（内存配置）

这些变量允许完全通过环境变量配置模型，无需修改配置文件：

| 变量 | 说明 | 默认值 |
|----------|-------------|---------|
| `RIPPERDOC_BASE_URL` | API 基础 URL | - |
| `RIPPERDOC_API_KEY` | API 密钥 | - |
| `RIPPERDOC_AUTH_TOKEN` | Bearer token 认证 | - |
| `RIPPERDOC_MODEL` | 模型名称（main 指针） | - |
| `RIPPERDOC_SMALL_FAST_MODEL` | 快速模型名称（quick 指针） | - |
| `RIPPERDOC_PROTOCOL` | 协议类型 | 自动检测 |

当设置 `RIPPERDOC_BASE_URL` 时，系统会在内存中创建 ModelProfile，不写入配置文件。协议类型可以显式指定，或根据 URL 和模型名称自动推断。
...
# 指定不同的 quick 模型
RIPPERDOC_SMALL_FAST_MODEL=haiku-model-name
```
