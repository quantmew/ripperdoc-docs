...
| 变量 | 说明 | 默认值 |
|----------|-------------|---------|
| `RIPPERDOC_EXIT_AFTER_STOP_DELAY` | Stdio 模式空闲后自动退出的延迟（秒） | - |
...
| `RIPPERDOC_ENABLE_TOOL_SEARCH` | 启用延迟 MCP 工具搜索（`true`/`false`） | - |

## 存储诊断

| 变量 | 说明 | 默认值 |
|----------|-------------|---------|
| `RIPPERDOC_STORAGE_DIAGNOSTICS` | 启用存储环境变量覆盖诊断 | `false` |
