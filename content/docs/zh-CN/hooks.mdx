...
| `PreToolUse` | 工具执行前 |
| `PostToolUse` | 工具完成后 |
| `PostToolUseFailure` | 工具失败或超时后 |
| `PermissionRequest` | 显示权限对话框时 |
| `UserPromptSubmit` | 用户提交消息时 |
| `Notification` | 状态通知 |
| `Stop` | 主代理完成时 |
| `SubagentStart` | 子代理（Task）启动前 |
| `SubagentStop` | 子代理（Task）完成时 |
| `PreCompact` | 上下文压缩前 |
| `SessionStart` | 会话开始或恢复时 |
| `SessionEnd` | 会话结束时 |
| `Setup` | 仓库初始化/维护（每个项目首次启动） |
...
适用事件：`PreToolUse`、`PermissionRequest`、`PostToolUse`、`PostToolUseFailure`。
...
Setup 会在每个项目首次 `SessionStart` 且 `source="startup"` 时触发。
...
| `block` | PostToolUse, PostToolUseFailure, UserPromptSubmit, Stop, SubagentStop | 阻止/防止停止 |
