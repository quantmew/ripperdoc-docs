...
这些变量允许完全在内存中创建模型配置，不依赖配置文件：

| 变量 | 说明 |
|----------|-------------|
| `RIPPERDOC_BASE_URL` | API 基础 URL（设置后将创建内存中的模型配置） |
| `RIPPERDOC_API_KEY` | API 密钥 |
| `RIPPERDOC_AUTH_TOKEN` | Bearer token 认证 |
| `RIPPERDOC_MODEL` | 模型名称 |
| `RIPPERDOC_SMALL_FAST_MODEL` | 快速模型名称（用于 quick 指针） |
| `RIPPERDOC_PROTOCOL` | 协议类型（anthropic、openai_compatible、gemini） |

当设置 `RIPPERDOC_BASE_URL` 时，系统会：
1. 自动检测协议类型（基于 URL 和模型名称）
2. 在内存中创建 ModelProfile（不写入配置文件）
3. 使用环境变量中的认证信息
