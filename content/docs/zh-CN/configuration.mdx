...
| `max_input_tokens` | 最大输入 token | 否 |
| `max_output_tokens` | 最大输出 token | 否 |
| `openai_tool_mode` | OpenAI 工具调用模式 | 否 |
| `supports_vision` | 覆盖图像/视觉支持（true/false） | 否 |
| `input_cost_per_million_tokens` | 费用追踪（输入） | 否 |
| `output_cost_per_million_tokens` | 费用追踪（输出） | 否 |

如果模型支持图像输入但自动识别失败，可设置 `supports_vision` 为 `true` 强制启用。
...
| `volcengine` | `api_key`, `model` | 火山引擎 Seed 2.0 |
| `aliyun` | `api_key`, `model` | 阿里云 |
| `stepfun` | `api_key`, `model` | 阶跃星辰 |
| `nvidia` | `api_key`, `model` | NVIDIA API 端点 |

### Provider 预设模式

为简化模型配置，你可以使用 provider 预设模式：

```bash
ripperdoc --model claude-sonnet  # 自动从模型名称检测 provider
```

系统会根据模型名称自动检测正确的 provider 和配置。
...
| `qwen` | Qwen 通义千问风格 |
| `gemini_openai` | Gemini OpenAI 兼容模式 |
| `openai` | OpenAI 风格思考 |

当未设置时，系统会根据提供商自动推断正确的思考模式。
...
API key 可以直接写在配置文件里。如果使用环境变量配置，Ripperdoc 只读取 `RIPPERDOC_*`（不会读取 `OPENAI_API_KEY` 等提供商变量）。

### RIPPERDOC_ 环境变量

这些变量会在当前会话中覆盖配置文件（不写入磁盘）。如果没有任何 profile，至少需要设置 `RIPPERDOC_MODEL`、`RIPPERDOC_PROTOCOL` 或 `RIPPERDOC_BASE_URL` 之一，才能构建模型配置。

| 变量 | 说明 |
|----------|-------------|
| `RIPPERDOC_BASE_URL` | API 基础 URL（`api_base` 覆盖） |
| `RIPPERDOC_API_KEY` | API 密钥（`api_key` 覆盖） |
| `RIPPERDOC_AUTH_TOKEN` | Auth token（`auth_token` 覆盖） |
| `RIPPERDOC_MODEL` | 模型名称覆盖 |
| `RIPPERDOC_SMALL_FAST_MODEL` | 快速模型名称（用于 quick 指针） |
| `RIPPERDOC_PROTOCOL` | 协议类型（anthropic、openai_compatible、gemini） |
