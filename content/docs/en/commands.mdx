...
| `--agent NAME` | Select a custom session agent |
| `--agents` | Open custom session agents management |
| `--debug` | Enable debug mode |
| `--disable-slash-commands` | Disable slash commands |
| `--replay-user-messages` | Replay user messages when resuming sessions |
...
Manage custom session agents and agent definitions (interactive UI by default).

```
> /agents                  # Open agents management UI
> /agents tui
> /agents list
> /agents use <name>       # Select an agent for current session
```

#### /plugins

Manage plugins with marketplace support (interactive UI by default).

```
> /plugins                 # Open plugins management UI
> /plugins tui
> /plugins list
