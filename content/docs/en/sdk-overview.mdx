...
options = RipperdocAgentOptions(
    ...
    sdk_url=None,                   # WebSocket URL for remote SDK transport
    replay_user_messages=False,     # Replay user messages when resuming sessions
)
```
...
```python
from ripperdoc_agent_sdk import RipperdocAgentOptions, AgentDefinition
...
async def handle_failure(event, input_data):
    print(f"Tool {input_data['tool_name']} failed: {input_data['error']}")
    return {"additionalContext": "Consider trying a different approach"}

async def handle_notification(event, input_data):
    print(f"Notification: {input_data['message']}")
    return {}

async def handle_permission_request(event, input_data):
    print(f"Permission requested for {input_data['tool_name']}")
...
options = RipperdocAgentOptions(
    hooks={
        ...
        "PostToolUseFailure": [
            HookMatcher(callback=handle_failure, tool_pattern="*")
        ],
        "Notification": [
            HookMatcher(callback=handle_notification, notification_type="*")
        ],
        "PermissionRequest": [
            HookMatcher(callback=handle_permission_request, tool_pattern="*")
        ]
    }
)
```
...
**Note**: As of v0.0.4, the Python SDK supports most hook events. The following events are **not yet supported**:
- `SessionStart`, `SessionEnd`
- `Setup`

Available hook events in the Python SDK v0.0.4+:

- `PreToolUse` - Before a tool is called (includes `tool_use_id`)
- `PostToolUse` - After a tool completes (includes `tool_use_id`, `tool_response`)
- `PostToolUseFailure` - After a tool fails (includes `error`, `is_interrupt`)
- `UserPromptSubmit` - When user submits a prompt
- `Stop` - When a stop is requested
- `SubagentStart` - Before a subagent starts (includes `agent_id`, `agent_type`)
- `SubagentStop` - When a subagent stops (includes `agent_id`, `agent_transcript_path`, `agent_type`)
- `PreCompact` - Before conversation compaction
- `Notification` - For status notifications (includes `message`, `title`, `notification_type`)
- `PermissionRequest` - When permission dialog is shown (includes `permission_suggestions`)
