...
| `RIPPERDOC_RESUME_REPLAY_MAX_MESSAGES` | Max messages to replay on resume (-1 for all, 0 to skip) | `120` |
...
| Variable | Description | Default |
|----------|-------------|---------|
| `RIPPERDOC_EXIT_AFTER_STOP_DELAY` | Auto-exit after idle delay in stdio mode (seconds) | - |
...
| `RIPPERDOC_ENABLE_TOOL_SEARCH` | Enable deferred MCP tool search (`true`/`false`) | - |
...
| Variable | Description | Default |
|----------|-------------|---------|
| `RIPPERDOC_STORAGE_DIAGNOSTICS` | Enable storage env override diagnostics | `false` |
...
RIPPERDOC_AUTOCOMPACT_PCT_OVERRIDE=80

# Stdio mode auto-exit
RIPPERDOC_EXIT_AFTER_STOP_DELAY=300

# MCP tool search control
RIPPERDOC_ENABLE_TOOL_SEARCH=true

# Storage diagnostics
RIPPERDOC_STORAGE_DIAGNOSTICS=true
