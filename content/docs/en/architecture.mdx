...
```
Event Flow:
1. Setup → Repository setup/maintenance (first startup per project)
2. UserPromptSubmit → User sends message
3. PreToolUse → Before tool execution
4. PermissionRequest → When permission needed
5. PostToolUse → After tool completes
6. PostToolUseFailure → After tool fails or times out
7. SubagentStart → Before a subagent starts
8. SubagentStop → When a subagent completes
9. Stop → Agent finishes responding
10. PreCompact → Before context compaction
11. SessionStart/SessionEnd → Session lifecycle
12. Notification → Status notifications
13. WorktreeCreate/WorktreeRemove → Worktree lifecycle
```
...
30+ tool implementations for file operations, search, execution, task management, team coordination, etc.
