...
Includes cache read/write tokens when available (e.g., Anthropic cache).
...
| `RIPPERDOC_REPLAY_USER_MESSAGES` | Replay user messages when resuming sessions | `false` |
...
Ripperdoc v0.5.1+ supports remote control bridge for distributed agent architectures:
...
Features:
- Explicit ACK protocol for reliable message delivery
- Session token refresh for long-running sessions
- Remote WebSocket SDK transport via `--sdk-url` option
