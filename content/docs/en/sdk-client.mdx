...
| `agents` | `Dict[str, AgentDefinition]` | `None` | Programmatic agents |
...
```python
from ripperdoc_agent_sdk import AgentDefinition
...
Available hook events (v0.0.4+):

```python
from ripperdoc_agent_sdk import HookEvent
...
class PreToolUseHookInput(BaseHookInput):
    ...
    tool_use_id: str              # Unique tool call identifier
```
...
class PostToolUseHookInput(BaseHookInput):
    ...
    tool_name: str
    tool_input: dict[str, Any]
    tool_response: Any            # Tool's response/output
    tool_use_id: str
```

#### PostToolUseFailureHookInput (v0.0.4+)
```python
class PostToolUseFailureHookInput(BaseHookInput):
    ...
    is_interrupt: NotRequired[bool]  # True if user interrupted
```

#### NotificationHookInput (v0.0.4+)
```python
class NotificationHookInput(BaseHookInput):
    hook_event_name: Literal["Notification"]
    message: str                   # Notification message
    title: NotRequired[str]        # Optional title
    notification_type: str         # Type identifier
```

#### SubagentStartHookInput (v0.0.4+)
```python
...
class SubagentStopHookInput(BaseHookInput):
    ...
    agent_transcript_path: str     # Path to agent transcript
    agent_type: str                # Agent type name
```

#### PermissionRequestHookInput (v0.0.4+)
```python
class PermissionRequestHookInput(BaseHookInput):
    hook_event_name: Literal["PermissionRequest"]
    tool_name: str
    tool_input: dict[str, Any]
    permission_suggestions: NotRequired[list[Any]]  # Suggested rules
```
...
#### PostToolUseFailureHookSpecificOutput (v0.0.4+)
```python
class PostToolUseFailureHookSpecificOutput(TypedDict):
    hookEventName: Literal["PostToolUseFailure"]
    additionalContext: NotRequired[str]  # Context to add to conversation
```

#### PermissionRequestHookSpecificOutput (v0.0.4+)
```python
class PermissionRequestHookSpecificOutput(TypedDict):
    hookEventName: Literal["PermissionRequest"]
    decision: dict[str, Any]       # Permission decision object
```
